...
def filter_extracted_text(raw_elements, llm_summary): 
    ...
    def generate_element_html(element, is_summary):
        ...
        if type(element) in image_element_items: 
            summary_text = element.metadata.summary if is_summary and hasattr(element, 'metadata') else sanitized_element
...
def edit_json(llm_elements):
    for i, element in enumerate(llm_elements): 
        if type(element) in text_element_items:
            edited_text = st.text_area(label="-", value=element.text, label_visibility="collapsed", key=i)
            element.text = edited_text
        if type(element) in image_element_items:
            edited_image = st.text_area(label="-", value=element.metadata.summary, label_visibility="collapsed", key=i)
            element.metadata.summary = edited_image
        if type(element) in table_element_items: 
            edited_table = st.text_area(label="-", value=element.text, label_visibility="collapsed", key=i)
